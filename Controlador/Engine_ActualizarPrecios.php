...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($imagen['size'] > 0) {
        ...
        $sql = "UPDATE precios SET NombreEmpresa = ?, Material = ?, Compra = ?, Venta = ?, ImagenMaterial = ? WHERE Id = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("sssssi", $nombreempresa, $material, $compra, $venta, $imagen_base, $campaign_id);
    } else {
        // Actualizar los datos de la campaña sin cambiar la imagen
        $sql = "UPDATE precios SET NombreEmpresa = ?, Material = ?, Compra = ?, Venta = ? WHERE Id = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("ssssi", $nombreempresa, $material, $compra,$venta, $campaign_id);
    }

    // Ejecutar la consulta preparada
    if ($stmt->execute()) {
        // Redirigir de vuelta a la página principal de campañas con un mensaje de éxito
        header("Location: ../Vistas/Form_EditarPrecios.php?mensaje=El material se actualizó correctamente.");
        exit;
    } else {
